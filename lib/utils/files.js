...
function validate(source,thumb,{outdated=true}={}){
    ...
    if(outdated){
      sequence.push(new Promise(function(resolve, reject) {
        fs.readFile(thumb,function(err,data){
          ...
          //We floor comparison as it's not always stored to ms precision
          if(typeof thumbTime !== "undefined" && Math.floor(thumbTime.getTime()/1000) === Math.floor(origMtime.getTime()/1000)){
            resolve(true);
          }else{
            resolve(false);
          }
        ...
        return new Promise(function(res, rej) {
          fs.readFile(thumb,function(err,data){
            ...
            try{
              ...
              //Files that are not valid PNG are deleted
              return res(thumb);
            }
